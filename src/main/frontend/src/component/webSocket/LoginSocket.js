...
export default function LoginSocket(props) {
    ...
    if( !ws.current ){      // 3.클라이언트소켓이 접속이 안되어있을때. [ * 유효성검사 ]
                ...
                ws.current.onmessage = (e) => {  console.log('서버소켓으로 메시지 받음');
                // 전달받은 메시지 확인
                console.log( e);  console.log( e.data );

                let data = JSON.parse( e.data ); // 전달받은 값(코드)
                let member = JSON.parse(sessionStorage.getItem('member')); // 멤버 정보
                let alertMsg = ''; // 코드에 따라 전달할 메시지 넣을 객체
                ...
                // 메시지 넣고 랜더링
                setMsg( (msg)=> [...msg , alertMsg  ] );
                }
            }
    } , [])
    // 메시지 들어올때마다 스크롤바 하단으로
    useEffect ( () => {
